          ...
          <div key={field.id} className="rounded-md border border-white/10 p-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="text-sm">Answer type<select className={fieldClass} value={field.type} onChange={(e) => update(index, { type: e.target.value as TemplateField["type"] })}>
                <option value="text">Short text</option><option value="textarea">Long text</option><option value="select">Single choice</option><option value="checkboxes">Multiple choice</option>
              </select></label>
            ...
            {["select", "checkboxes"].includes(field.type) && <label className="mt-4 block text-sm">Options (one per line)<textarea className={fieldClass} value={field.options.join("\n")} onChange={(e) => update(index, { options: e.target.value.split("\n") })} /></label>}
            <div className="mt-4 flex justify-between gap-4"><label className="text-sm"><input type="checkbox" checked={field.required} onChange={(e) => update(index, { required: e.target.checked })} /> Required</label><button type="button" onClick={() => setFields((old) => old.filter((_, i) => i !== index))} className="text-sm text-purple">Remove question</button></div>
          </div>
        ))}
        <div className="flex flex-wrap gap-4">
          <button type="button" disabled={fields.length >= 60} className={`text-sm text-purple ${focusRing}`} onClick={() => setFields((old) => [...old, { id: `q_${crypto.randomUUID().replaceAll("-", "")}`, label: "", section: "Business details", type: "text", required: false, options: [], hint: "" }])}>+ Add question</button>
